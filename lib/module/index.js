...
export default class SwanSDK {
  appId = '';
  deviceId = '';
  webSDKUrl = 'https://rnvpf-49-37-160-192.a.free.pinggy.link';
  ecomEventUrl = 'https://e-commerce.azurefd.net/api/trackEvent';
  ecomDeviceUrl = `https://e-commerce.azurefd.net/api/device`;
  notificationAckUrl = `${this.webSDKUrl}/api/post-in-app-notification-sdk-ack`;
      ...
      try {
        ...
        this.db.transaction(function (txn) {
          txn.executeSql(`DELETE FROM ${tableName} WHERE ${key} = ?;`, [value], function () {
            console.log(`Record with key ${key} and value ${value} deleted`);
      ...
      try {
        ...
        this.db.transaction(function (txn) {
          txn.executeSql(`DELETE FROM ${tableName};`, [], function () {
            console.log(`All records deleted from table ${tableName}`);
            resolve(); // Resolve the promise on successful deletion
          }, function (_, error) {
            console.error('Error executing SQL:', error);
            reject(error); // Reject the promise on SQL execution error
  ...
  async getDeviceId(pushNotificationToken) {
    try {
      ...
      if (encodedCredentials) {
        const decodedCredentials = JSON.parse(Base64.decode(encodedCredentials));
        console.log('decodedCredentials1', decodedCredentials);
      ...
      const encodedUserDetails = Base64.encode(JSON.stringify(userDetails));
      await AsyncStorage.setItem('swanCredentials', encodedUserDetails);
      return deviceId;
    } catch (error) {
  ...
  async setPushNotificationPermission(permission) {
    try {
      // Check if the device ID is already stored in AsyncStorage
      const encodedCredentials = await AsyncStorage.getItem('swanCredentials');
      console.log('pushCreds', encodedCredentials);
      if (!encodedCredentials) {
      ...
      let decodedCredentials = JSON.parse(Base64.decode(encodedCredentials));
      ...
      if (!decodedCredentials.pushPermission || decodedCredentials.pushPermission !== permission) {
        ...
        const encodedUserDetails = Base64.encode(JSON.stringify(decodedCredentials));
        await AsyncStorage.setItem('swanCredentials', encodedUserDetails);
      }
    } catch (error) {
  ...
  async identify(cdid) {
    try {
      ...
      const data = {
        ...
        subscription: decodedCredentials.pushNotificationToken ? {
          pushNotificationToken: decodedCredentials.pushNotificationToken,
          subscribed: decodedCredentials.pushPermission,
          lastLoginPlatform: Platform.OS
        } : null,
        status: decodedCredentials.pushNotificationToken ? decodedCredentials.pushPermission ? 'updated' : 'revoked' : null
      };
      const response = await this.sendToSwan(url, data, true);
      ...
      console.log('decodedCredsForIdentify', decodedCredentials);
      ...
      // Retrieve existing credentials from AsyncStorage
      const encodedCredentials1 = await AsyncStorage.getItem('swanCredentials');

      // Decode the credentials if they exist
      let decodedCredentials1 = JSON.parse(Base64.decode(encodedCredentials1 || ''));
      console.log('decodedCrendentials1', decodedCredentials1);
      console.log('CDID successfully updated and stored.');
    } catch (error) {
  ...
  async unlinkDevice() {
    try {
      ...
      const encodedUserDetails = Base64.encode(JSON.stringify(decodedCredentials));
      await AsyncStorage.setItem('swanCredentials', encodedUserDetails);
      await AsyncStorage.removeItem('_swanSessionId');
    } catch (error) {
    ...
    try {
      ...
      if (sessionBase64) {
        ...
        if (currentTime - lastActiveTime < sessionDuration) {
          ...
          console.log('sessionId', sessionDoc.sessionId);
      ...
      console.log('newSessionDoc', newSessionDoc);
      console.log('newSessionId', newSessionDoc.sessionId);
  ...
  async trackEvent(eventName, eventData) {
    try {
      ...
      payload.data.osModal = Platform.OS;
  ...
  async processNotification(notificationList) {
    ...
    console.log('dispkaLimit', displayLimit);
    ...
    console.log('newDisplayLimit', newDisplayLimit, displayUnlimited);
  ...
  async fetchNotificationFromAPI() {
    try {
      const response = await fetch(`${this.webSDKUrl}/api/post-in-app-notification-to-sdk?appId=${this.appId}&mobile=919019467907`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
  ...
  async fetchNotificationFromDB() {
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        console.log('commId', rows[i].commId);
  ...
  async getNotificationComponent() {
    ...
    if (!lastNotificationFetchFromApi || (await this.isGreaterThanTimePeriod(lastNotificationFetchFromApi, 1))) {
    ...
    if (!lastNotificationFetchFromDB || (await this.isGreaterThanTimePeriod(lastNotificationFetchFromDB, 1))) {
  ...
  async isGreaterThanTimePeriod(lastFetchTimeString, timePeriod) {
    ...
    if (timeDifference > timePeriod * 60 * 1000) {
      console.log('More than 30 minutes have passed since the last fetch.');
      return true;
    } else {
      console.log('Less than 30 minutes have passed since the last fetch.');
      return false;
    }
  }
}
