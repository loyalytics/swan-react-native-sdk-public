...
class SwanSDK {
  ...
  async identify(cdid) {
    try {
      ...
      // Retrieve existing credentials from AsyncStorage
      const encodedCredentials1 = await _asyncStorage.default.getItem('swanCredentials');

      // Decode the credentials if they exist
      let decodedCredentials1 = JSON.parse(_reactNativeBase.default.decode(encodedCredentials1 || ''));
